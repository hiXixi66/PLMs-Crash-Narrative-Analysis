...
# for i in {5,10,15,20, 30,40}:  # 5%, 10%, 15%, 30%, 40%
    # EXCEL_PATH = f"data/processed_data/case_info_2021_{i}perc_bias.xlsx"
    # ======== 配置 ========
for i in range(200,601,200):
    ...
    MAX_LENGTH = 256
    OUTPUT_DIR = f"./models/bert-ft-MANCOLL-limit-sample-test/limit-samples-{i}-new"
    ...
    # 清理空值
    df = df[[TEXT_COL, LABEL_COL]].dropna().reset_index(drop=True).iloc[:i]
