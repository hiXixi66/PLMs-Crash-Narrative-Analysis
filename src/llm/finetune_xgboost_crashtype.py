...
# ======== case-level 评估并导出 ========
def eval_and_print_xgb(name, df_crash, df_gv, df_dataset, model, vectorizer, label2id, output_dir):
    if df_dataset is None or len(df_dataset) == 0:
        return None
...
for i, (n_estimators, max_depth, learning_rate, min_child_weight, subsample, colsample_bytree, reg_lambda) in enumerate(all_settings, 1):
    params = {
...
# ======== case-level 评估并导出 ========
test_metrics = eval_and_print_xgb(
    "Test",
    df_crash_test,
    df_gv_test,
    df_test_extra,
