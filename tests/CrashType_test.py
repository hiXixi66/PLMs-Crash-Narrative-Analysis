...
def safe_to_excel(df: pd.DataFrame, path: str):
    ...
    # 写到同目录的临时文件，再原子替换
    with tempfile.NamedTemporaryFile(dir=str(path.parent), suffix=".xlsx", delete=False) as tmp:
...
def append_row_to_excel(row_data, file_path):
    """避免多次半写；每次读旧表 + 追加 + 原子覆盖写"""
    new_row_df = pd.DataFrame([row_data])
    p = Path(file_path)
    if p.exists() and p.stat().st_size > 0:
        # 显式指定引擎，兼容性更好
        existing_df = pd.read_excel(p, engine="openpyxl")
        updated_df = pd.concat([existing_df, new_row_df], ignore_index=True)
    else:
        updated_df = new_row_df
    safe_to_excel(updated_df, file_path)
...
def vehicle_crashtype_classification_with_classifier(classifier,  config_crashtype_df, file_path, output_excel_path):
    ...
    y_true = []
    y_pred = []
    # save_path = "reports/crash-type-test/crash_type_resultsqkv-temp5.csv"
    ...
    for idx, row in df.iterrows():
        ...
        if number_of_vehicles >0:
            ...
            for i in range(1, number_of_vehicles + 1):
                ...
                # crashtype = classify_crashtype(i,summary, crashtype_class)
                # print(f"Predicted values for vehicle {i}: {crashcat}, {crashconf}, {crashtype}")
                pred.append((case_id, i, crashcat, crashconf, crashtype_bert if LLM.provider == "BERT" else crashtype))
                # print(f"Predicted values for vehicle {i}: {pred}")
                # correct_flag = 1 if crashtype == gt_type else 0
...
if __name__ == "__main__":
    ...
    # LLM = LLM_HPC(model_name="llama3-3b", provider="transformers", max_new_tokens=2)
    ...
    output_excel_path = "reports/crash-type-test/results/evaluation_bert-2900.xlsx"
